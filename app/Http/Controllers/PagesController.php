...
class PagesController extends Controller
{
    ...
    public function category($cleUrl)
    {
        ...
        $offset = $offset > count($products) || $offset < 0 ? 0 : $offset;
        if ($offset == 0)
        {
            $page = 1;
        }
        $products = array_slice($products, $offset, self::LIMIT);
        if (count($products) > 0)
        {
        ...
        return view('portal.pages.category', ["paramController" => 'ProductParametersController@getProductParametersByCleUrl', "page" => $page, "paramUrl" => $paramUrl, "offset" => $offset, "limit" => self::LIMIT, "sort_selected" => $this->getSortSelected(), 'sort' => self::SORT, 'pages' => $pages, 'products' => $products, 'bread_list' => $breadList, 'page_name' => $pageName, 'url' => $cleUrl, 'categories' => $categories, 'title' => $title, 'user_lang' => App::getLocale(), 'languages' => Language::all()]);
    }
    ...
    public function offers($url)
    {
        ...
        $pageReviews = is_numeric(Input::get('page_reviews', 1)) ? Input::get('page_reviews', 1) : 1;
        $offsetReviews = ($pageReviews * self::LIMIT) - self::LIMIT;
        $pageOffers = is_numeric(Input::get('page_offers', 1)) ? Input::get('page_offers', 1) : 1;
        ...
        $offers = DB::select(self::SQL_OFFERS_SELECT_OFFERS, [$product->put_id, App::getLocale(), App::getLocale(), $product->put_id]);
        ...
        $reviews = DB::select(self::SQL_OFFERS_SELECT_REVIEWS_BY_PUT_ID, [$product->put_id, App::getLocale(), $product->put_id, App::getLocale()]);
        $offsetReviews = $offsetReviews > count($reviews) || $offsetReviews < 0 ? 0 : $offsetReviews;
        $offsetOffers = $offsetOffers > count($offers) || $offsetOffers < 0 ? 0 : $offsetOffers;
        ...
        $offers = array_slice($offers, $offsetOffers, self::LIMIT);
        $reviews = array_slice($reviews, $offsetReviews, self::LIMIT);
    ...
    public function companies()
    {
        ...
        $offset = ($page * self::LIMIT) - self::LIMIT;
        $offset = $offset > $companies->count() || $offset < 0 ? 0 : $offset;
        if ($offset == 0)
        {
            $page = 1;
        }
        ...
        return view('portal.pages.companies', ["page" => $page, 'lge_id' => Language::where('lge_abbreviation', '=', App::getLocale())->first()->lge_id, 'pages' => $pages, 'user_lang' => App::getLocale(), 'languages' => Language::all(), "page_name" => $pageName, "breads" => $breadCrums, "companies" => $companies, "title" => $title]);
    }
    ...
    public function company($actId)
    {
        ...
        $page = is_numeric(Input::get('page', 1)) ? Input::get('page', 1) : 1;
        $offset = ($page * self::LIMIT) - self::LIMIT;
        $offset = $offset > $reviewsCount || $offset < 0 ? 0 : $offset;
        if ($offset == 0)
        {
            $page = 1;
        }
        ...
        return view('portal.pages.company', ['reviews_count' => $reviewsCount, 'page' => $page, 'pages' => $pages, 'rating' => $rating, 'reviews' => $reviews, 'store_controller' => 'ReviewController@storeCompany', 'url' => ["company" => $actId], 'lge_id' => $lgeId, 'user_lang' => App::getLocale(), 'languages' => Language::all(), "page_name" => $pageName, "breads" => $breadCrums, "company" => $company, "title" => $title]);
    }
    ...
    {
        switch (Input::get('sort', "sort_name"))
        {
            case "sort_price_min":
                return "sort_price_min";
                break;
            case "sort_price_max":
                return "sort_price_max";
                break;
            case "sort_relevance":
                if (Input::get('search' ,false) !== false)
                {
